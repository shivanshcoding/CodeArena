...
export default function CompleteProfile() {
  ...
  // ✅ Username check using backend
  const checkUsername = async (username) => {
    if (!username) return false;
    try {
      const res = await axios.post('/profile/check-username', { username });
        ...
        <div>
          ...
          <input
            {...register('username', {
              ...
              validate: async (val) =>
                (await checkUsername(val)) || 'Username is already taken'
            })}
            className="input"
            placeholder="e.g. shivanshrana"
          />
          {errors.username && (
